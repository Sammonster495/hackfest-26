...
export async function createParticipationPayment({
    ...
    const siteSettings = Array.isArray(rawSettings) ? rawSettings[0] : rawSettings;

    if (!siteSettings?.paymentsOpen) {
        throw new AppError("PAYMENTS_NOT_OPEN", 403, {
            title: "Payments not open",
            description: "The payment portal is currently closed. Please try again later.",
        });
    }

    const team = await db.query.teams.findFirst({
        where: eq(teams.id, teamId),
        columns: { id: true, leaderId: true, isCompleted: true, paymentStatus: true },
    });
