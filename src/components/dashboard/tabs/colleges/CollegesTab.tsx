    ...
    const fetchAllCounts = () => {
        fetch("/api/dashboard/college-requests/count")
            ...
            .then((data) => {
                ...
                if (typeof data.collegesCount === "number") setCollegesCount(data.collegesCount);
            })
            .catch(console.error);
    };
...
function CollegeRequestsSubTabs({ counts }: { counts: { Pending: number; Approved: number; Rejected: number } }) {
    const [subTab, setSubTab] = useState<"Pending" | "Approved" | "Rejected">("Pending");
    const [hasInitialized, setHasInitialized] = useState(false);

    useEffect(() => {
        if (!hasInitialized && counts.Pending === 0 && (counts.Approved > 0 || counts.Rejected > 0)) {
            setSubTab("Approved");
