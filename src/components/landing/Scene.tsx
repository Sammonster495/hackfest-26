...
import { ScrollControls, useScroll, useTexture, Scroll, Html } from "@react-three/drei";
...
function LoadingScreen({ progress }: { progress: number }) {
    return (
        ...
        >
            ...
            />
            <motion.div
                className="mt-8 h-1 w-64 bg-white/20 rounded-full overflow-hidden"
            >
                <motion.div
...
function LandingContent({ setPages }: { setPages: (pages: number) => void }) {
    ...
    return (
        <div ref={ref} className="w-full text-white no-scrollbar pointer-events-auto">
            ...
            >
                ...
                <div className="absolute bottom-12">
                    <p className="text-sm font-semibold tracking-widest uppercase opacity-80 animate-pulse">Scroll to Dive</p>
                </div>
            </motion.section>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                        {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                            <motion.div
                                ...
                                viewport={{ once: true }}
                            >
                                <span className="text-cyan-400 font-mono text-lg group-hover:scale-110 transition-transform">Sponsor {i}</span>
